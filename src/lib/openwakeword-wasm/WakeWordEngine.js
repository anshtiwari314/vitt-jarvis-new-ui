...
const MIC_WORKLET_URL = '/openwakeword/mic-worklet.js';
...
export class WakeWordEngine {
    ...
    async load() {
        ...
        try {
            ...
            await this._releaseModelSessions(createdSessions);
            this._melspecModel = null;
            this._embeddingModel = null;
            this._vadModel = null;
            this._keywordModels = null;
            this._loaded = false;
            throw err;
        } finally {
            this._loading = false;
    ...
    async release() {
        await this.stop();
        await this._releaseModelSessions([
            this._melspecModel,
            this._embeddingModel,
            this._vadModel,
            ...(this._keywordModels ? Object.values(this._keywordModels).map((k) => k.session) : []),
        ]);
        this._melspecModel = null;
        this._embeddingModel = null;
        this._vadModel = null;
        ...
        this._loaded = false;
        this._loading = false;
        this._debug('All ONNX sessions released');
    }

    async _releaseModelSessions(sessions) {
    ...
    async start({ deviceId, gain = 1.0 } = {}) {
        ...
        try {
            this._audioContext = new AudioCtx({ sampleRate: this.config.sampleRate });
        } catch {
        ...
        this._workletNode = new AudioWorkletNode(this._audioContext, 'pcm-worklet');

        this._workletNode.port.onmessage = (event) => {
            const pcm = event.data;
            if (!pcm || pcm.length !== this.config.frameSize) return;
        ...
        this._workletNode.connect(sink);
        sink.connect(this._audioContext.destination);
        this._sink = sink;
    ...
    get sampleRate() {
        return this._audioContext ? this._audioContext.sampleRate : null;
    }
    ...
    async _runInference(chunk, isSpeechActive, emitEvents) {
        ...
        while (this._melBuffer.length >= 76) {
            ...
            for (const name of Object.keys(this._keywordModels)) {
                ...
                if (emitEvents) {
                    this._emitter.emit('score', { keyword: name, score });
                }

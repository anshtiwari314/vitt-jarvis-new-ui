...
async function waitForGlobalOrt(timeoutMs = 8000): Promise<void> {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    const g = globalThis as { ort?: { InferenceSession?: { create?: unknown } } }
    if (g.ort?.InferenceSession?.create) return
...
export function useWakeWord({
  ...
  useEffect(() => {
    ...
    async function loadModels() {
      try {
        ...
        // On /#/mainpage, VAD loads CDN ort first — share it to avoid dual ORT conflicts.
        await waitForGlobalOrt()

        // Vendored openwakeword uses src/lib/onnxruntime-web-shim.js for ORT.
        ...
        const oww = await OpenWakeWord.create({
          ...
          onUtterance: onUtteranceRef.current
            ? (e: any) => onUtteranceRef.current!(e)
            : undefined,
        })
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      oww.onDetection = (e: any) => {
        setLastDetection(e)
        onDetectionRef.current?.(e)
      }
      if (onUtteranceRef.current) {
        oww.onUtterance = (e: any) => onUtteranceRef.current!(e)
      }
